  ...
  try {
    ...
    for (const event of factoryResponse.events) {
      ...
      if (topic0 === "factory" && topic1 === "circle_created") {
        await handleFactoryCircleCreated(event);
      } else if (topic0 === "reputation" && topic1 === "increment") {
        await handleReputationIncrement(event);
      }
    }
    ...
    if (circles.length > 0) {
      ...
      for (const event of circleResponse.events) {
        ...
        switch (topic1) {
          case "joined":       await handleCircleJoined(contractId, event);      break;
          case "active":       await handleCircleActive(contractId);              break;
          case "contributed":  await handleCircleContributed(contractId, event); break;
          case "payout":       await handleCirclePayout(contractId, event);      break;
          case "default":      await handleCircleDefault(contractId, event);     break;
          case "completed":    await handleCircleCompleted(contractId);          break;
        }
      }
    }
  } catch (err) {
